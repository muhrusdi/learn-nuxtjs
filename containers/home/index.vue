<template>
  <div>
    <box title="Profile Information" description="Your personal data">
      <template v-if="!isEdit" v-slot:rightContent>
        <nuxt-link :to="'?edit=' + user_id">
          ...
          <div class="w-1/2">
            <div>
              ...
              >
                ...
                <template  v-if="isEdit" v-slot:button>
...
export default {
  data() {
    return {
      items: [{text: "Male", value: "0"}, {text: "Female", value: "1"}],
      user_id: "",
      fields: {
        name: "",
        gender: "",
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (!this.$v.$error) {
        this.loading = true
        this.$store.dispatch("profile/postProfile", {
